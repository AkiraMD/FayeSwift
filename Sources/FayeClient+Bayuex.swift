...
extension FayeClient {
    ...
    func connect() {
        writeOperationQueue.sync { [unowned self] in
            let dict:[String:AnyObject] = [Bayeux.Channel.rawValue: BayeuxChannel.Connect.rawValue as AnyObject, Bayeux.ClientId.rawValue: self.fayeClientId! as AnyObject, Bayeux.ConnectionType.rawValue: BayeuxConnection.WebSocket.rawValue as AnyObject, Bayeux.Advice.rawValue: ["timeout": self.timeOut] as AnyObject]
    ...
    func publish(_ data:[String:AnyObject], channel:String) {
        writeOperationQueue.sync { [weak self] in
            if let clientId = self?.fayeClientId, let messageId = self?.nextMessageId(), self?.fayeConnected == true {
                ...
                // every message needs auth message headers too
                if let headerDict = self?.messageHeaderDict {
                    for (key, value) in headerDict {
                        if let keyString = key as? String {
                            dict[keyString] = value as AnyObject
                        }
                    }
                }
